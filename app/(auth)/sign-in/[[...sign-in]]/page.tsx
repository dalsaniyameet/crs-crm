...
function PunchSection() {
  ...
  const verify = async (e: React.FormEvent) => {
    ...
    try {
      ...
      if (!res.ok || !data.found) {
        setVerifyErr("Employee not found. Please contact your admin.");
      } else {
  ...
  if (done) return (
    <motion.div initial={{ opacity: 0, scale: 0.95 }} animate={{ opacity: 1, scale: 1 }}
      ...
      {done.wh && <div className="text-muted-foreground text-xs">{(done.wh as number).toFixed(1)}h worked</div>}
      <button onClick={() => { setDone(null); setVerified(null); setEmail(""); localStorage.removeItem("crs_punch_email"); }}
        className="w-full py-2 rounded-xl bg-white/10 text-white text-sm hover:bg-white/20 transition-all">Done</button>
    </motion.div>
  );
