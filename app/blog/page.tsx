        ...
        <div style={{ display: "flex", flexDirection: "column", gap: 20 }}>
          {POSTS.map(post => (
            <Link key={post.slug} href={`/blog/${post.slug}`} style={{ textDecoration: "none" }}>
              <article style={{ padding: "28px", borderRadius: 20, background: "rgba(255,255,255,0.02)", border: `1px solid ${post.color}18`, transition: "all 0.2s", cursor: "pointer" }}
                onMouseEnter={e => (e.currentTarget.style.borderColor = `${post.color}40`)}
                onMouseLeave={e => (e.currentTarget.style.borderColor = `${post.color}18`)}>
